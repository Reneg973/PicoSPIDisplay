...
#define SPI_BAUDRATE 35 * 1000 * 1000
...
#undef COMMAND_LOCK_INIT2
    ...
    sendCommand(SSD1351_CMD_MUXRATIO, 127);    // number rows to be muxed
    sendCommand(SSD_CMD_SETREMAP, 0b00100000);    //RGB format
...
//    sendCommand(SSD1331_CMD_MUXRATIO, 11);    // number rows to be muxed
    sendCommand(SSD_CMD_SETREMAP, 0b00100000);     //RGB format; enable odd/even split
    sendCommand(SSD_CMD_STARTLINE, 0);
    sendCommand(SSD_CMD_DISPLAYOFFSET, 0x0);
...
//    sendCommand(SSD1331_CMD_POWERSAVE_ON, 0xB);
  ...
  void fill(uint16_t color, bool wait=true)
  {
    ...
    else if (auto chan = DMAChannel(true))
    {
      ...
      {
        _dmaTransfer = Transaction([this, chan = std::move(chan)]()
          {
            dma_channel_wait_for_finish_blocking(chan);
            // when DMA finished, this doesn't mean SPI has finished, so wait for it
            _spiOut.waitTransfer();
  ...
  void bitblt(uint x0, uint y0, uint x1, uint y1, uint16_t const* data, bool wait)
  {
...
{
  auto& p = *reinterpret_cast<SSD1351Priv*>(_priv);
  p.sendCommand(SSD_CMD_HORIZSCROLL, {0, 0, 0, 0, 1});
}
...
void SSD1351::bitblt(uint x0, uint y0, uint x1, uint y1, uint16_t const* data, bool wait)
{
  auto& p = *reinterpret_cast<SSD1351Priv*>(_priv);
  p.setRegion(x0, y0, x1, y1);
